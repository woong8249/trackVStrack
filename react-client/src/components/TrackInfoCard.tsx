        ...
        {('artists' in track && track.artists)
          ? (
            <div className='text-xs overflow-auto max-h-[5rem] flex flex-col gap-1'>
              <p className='text-[#3D3D3D]  w-[15rem] sm:max-w-[20rem] sm:w-[20rem] responsive-small-text'>{track.titleName}</p>
              <p className='text-[#9A9A9A] responsive-extra-small-text'>{track.artists.map((artist) => artist.artistName).join(', ')}</p>
              <p className=' text-[#707070] responsive-extra-small-text'>{track.releaseDate ? track.releaseDate.split('T')[0] : '정보 없음'}</p>
            </div>
